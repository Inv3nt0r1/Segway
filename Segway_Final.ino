...
float Yaw, Pitch, Roll;
 ...
 DMSR=constrain(DMSR, -80, 80);
 DMSL=constrain(DMSL, -80, 80);
...
void Mcommand () {
  ...
  COcommand=constrain(COcommand, -80, 80);
...
void loop() 
{
  ...
  { 
     ...
     if(Roll < 0)         //Roll is negative for moving forward. If Roll becomes positive change it to Roll > 0
     {
      ...
      if(Pot > 800 || Pot < 700)      //If handle is bent left or right
      {
        
        if(Pot > 800)      //Turn left
        {
